...
class MyModal(niles.ui.Modal):
    def __init__(self) -> None:
        components = [
            niles.ui.TextInput(
                ...
                custom_id="name",
                style=niles.TextInputStyle.short,
                min_length=5,
                max_length=50,
            ),
            niles.ui.TextInput(
                ...
                custom_id="content",
                style=niles.TextInputStyle.paragraph,
                min_length=5,
                max_length=1024,
            ),
        ]
    ...
    async def callback(self, inter: niles.ModalInteraction) -> None:
        tag_name = inter.text_values["name"]
        tag_content = inter.text_values["content"]

        embed = niles.Embed(title=f"Tag created: `{tag_name}`")
        embed.add_field(name="Content", value=tag_content)
        await inter.response.send_message(embed=embed)

    async def on_error(self, error: Exception, inter: niles.ModalInteraction) -> None:
        await inter.response.send_message("Oops, something went wrong.", ephemeral=True)


@bot.slash_command()
async def create_tag(inter: niles.CommandInteraction):
...
@bot.slash_command()
async def create_tag_low(inter: niles.CommandInteraction):
    ...
    try:
        modal_inter: niles.ModalInteraction = await bot.wait_for(
            "modal_submit",
