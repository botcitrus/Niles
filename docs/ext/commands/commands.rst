...
Working with :ref:`Discord models <discord_model>` is a fairly common thing when defining commands, as a result the library makes
...
.. code-block:: python3
    ...
    @bot.command()
    async def joined(ctx, *, member: niles.Member):
...
.. code-block:: python3
    ...
    @bot.command()
    async def union(ctx, what: typing.Union[niles.TextChannel, niles.Member]):
        await ctx.send(what)


The ``what`` parameter would either take a :class:`niles.TextChannel` converter or a :class:`niles.Member` converter.
The way this works is through a left-to-right order. It first attempts to convert the input to a
:class:`niles.TextChannel`, and if it fails it tries to convert it to a :class:`niles.Member`. If all converters fail,
...
.. code-block:: python3
    ...
    @bot.command()
    async def slap(ctx, members: commands.Greedy[niles.Member], *, reason='no reason'):
...
.. code-block:: python3
    ...
    @bot.command()
    async def ban(ctx, members: commands.Greedy[niles.Member],
...
.. warning::
    ...
    For example, a signature expecting a :data:`typing.Optional` of a :class:`niles.Member` followed by a
...
.. code-block:: python3
    ...
    class BanFlags(commands.FlagConverter):
        members: List[niles.Member] = commands.flag(name='member', default=lambda ctx: [])
...
The flag converter is similar to prefix commands and allows you to use most types of converters
...
.. code-block:: python3
    ...
    class BanFlags(commands.FlagConverter):
        members: List[niles.Member] = commands.flag(name='member')
        reason: str
        days: int = 1
...
.. code-block:: python3
    ...
    @bot.command()
    async def info(ctx, *, member: niles.Member):
