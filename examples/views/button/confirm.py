...
class Confirm(niles.ui.View):
    ...
    @niles.ui.button(label="Confirm", style=niles.ButtonStyle.green)
    async def confirm(self, button: niles.ui.Button, inter: niles.MessageInteraction):
        await inter.response.send_message("Confirming...", ephemeral=True)
        self.value = True
        self.stop()

    # This one is similar to the confirmation button except sets the inner value to `False`.
    @niles.ui.button(label="Cancel", style=niles.ButtonStyle.grey)
    async def cancel(self, button: niles.ui.Button, inter: niles.MessageInteraction):
