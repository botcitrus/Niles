...
import niles
from niles.app_commands import Option, OptionChoice
from niles.channel import _channel_type_factory
from niles.enums import ChannelType, OptionType, try_enum_to_int
from niles.ext import commands
from niles.i18n import Localized
from niles.interactions import ApplicationCommandInteraction
from niles.utils import (
...
if TYPE_CHECKING:
    ...
    from niles.app_commands import Choices
    from niles.i18n import LocalizationValue, LocalizedOptional
    from niles.types.interactions import ApplicationCommandOptionChoiceValue
...
BotT = TypeVar("BotT", bound="niles.Client", covariant=True)
...
class ParamInfo:
    ...
    TYPES: ClassVar[Dict[type, int]] = {
        ...
        niles.abc.User:                                  OptionType.user.value,
        niles.User:                                      OptionType.user.value,
        niles.Member:                                    OptionType.user.value,
        Union[niles.User, niles.Member]:               OptionType.user.value,
        # channels handled separately
        niles.abc.GuildChannel:                          OptionType.channel.value,
        niles.Role:                                      OptionType.role.value,
        niles.abc.Snowflake:                             OptionType.mentionable.value,
        Union[niles.Member, niles.Role]:               OptionType.mentionable.value,
        Union[niles.User, niles.Role]:                 OptionType.mentionable.value,
        Union[niles.User, niles.Member, niles.Role]: OptionType.mentionable.value,
        float:                                             OptionType.number.value,
        niles.Attachment:                                OptionType.attachment.value,
        # fmt: on
    }
    ...
    def from_param(
        ...
        parsed_docstring: Optional[Dict[str, niles.utils._DocstringParam]] = None,
    ) -> Self:
    ...
    async def verify_type(self, inter: ApplicationCommandInteraction, argument: Any) -> Any:
        ...
        if (
            isinstance(argument, niles.User)
            and issubclass_(self.type, niles.Member)
            and not issubclass_(self.type, niles.User)
        ):
    ...
    def _parse_enum(self, annotation: Any) -> None:
        if isinstance(annotation, (EnumMeta, niles.enums.EnumMeta)):
    ...
    def _parse_guild_channel(
        self, *channels: Union[Type[niles.abc.GuildChannel], Type[niles.Thread]]
    ) -> None:
        ...
        self.type = niles.abc.GuildChannel
    ...
    def parse_annotation(self, annotation: Any, converter_mode: bool = False) -> bool:
        ...
        if self.large:
            ...
            isinstance(annotation, (EnumMeta, niles.enums.EnumMeta))
            ...
            if all(
                issubclass_(channel, (niles.abc.GuildChannel, niles.Thread)) for channel in args
            ):
        ...
        elif issubclass_(annotation, (niles.abc.GuildChannel, niles.Thread)):
    ...
    def parse_doc(self, doc: niles.utils._DocstringParam) -> None:
...
def collect_params(
    ...
    doc = niles.utils.parse_docstring(function)["params"]
...
def expand_params(command: AnySlashCommand) -> List[Option]:
    ...
    if issubclass_(parameters[inter_param].annotation, niles.GuildCommandInteraction):
