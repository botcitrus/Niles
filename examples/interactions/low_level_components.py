...
async def send_button(ctx: commands.Context):
    await ctx.send(
        "Here's a button!",
        components=niles.ui.Button(label="Click me!", custom_id="cool_button"),
    )
...
async def send_select(ctx: commands.Context):
    await ctx.send(
        "Here's a select!",
        components=niles.ui.StringSelect(options=["1", "2", "3"], custom_id="cool_select"),
    )
...
# row ordering is desired, simply store them in a list of lists or `niles.ui.ActionRow`s instead:
...
async def cool_button_listener(inter: niles.MessageInteraction):
...
async def cool_select_listener(inter: niles.MessageInteraction):
...
@bot.listen("on_button_click")
async def grid_listener(inter: niles.MessageInteraction):
