...
class CommonBotBase(Generic[CogT]):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.owner: Optional[niles.User] = None
        self.owners: Set[niles.TeamMember] = set()
    ...
    async def _fill_owners(self) -> None:
        ...
        app: niles.AppInfo = await self.application_info()  # type: ignore
        if app.team:
            self.owners = owners = {
                member
                for member in app.team.members
                # these roles can access the bot token, consider them bot owners
                if member.role in (niles.TeamMemberRole.admin, niles.TeamMemberRole.developer)
            }
    ...
    async def is_owner(self, user: Union[niles.User, niles.Member]) -> bool:
        """|coro|

        Checks if a :class:`~niles.User` or :class:`~niles.Member` is the owner of
        this bot.
        ...
        .. versionchanged:: 2.10
            Also takes team roles into account; only team members with the :attr:`~niles.TeamMemberRole.admin`
            or :attr:`~niles.TeamMemberRole.developer` roles are considered bot owners.
    ...
    def add_cog(self, cog: Cog, *, override: bool = False) -> None:
        ...
        if existing is not None:
            if not override:
                raise niles.ClientException(f"Cog named {cog_name!r} already loaded")
    ...
    def load_extensions(self, path: str) -> None:
        ...
        """
        for extension in niles.utils.search_directory(path):
    ...
    async def _watchdog(self) -> None:
        ...
        """
        if isinstance(self, niles.Client):
