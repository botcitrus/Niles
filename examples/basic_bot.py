...
"""An example to showcase the niles.ext.commands extension module.
...
@bot.command()
async def joined(ctx: commands.Context, member: niles.Member):
    """Says when a member joined."""
    if member.joined_at:
        # formats the join time/date like "5 years ago"
        date_str = niles.utils.format_dt(member.joined_at, "R")
    else:
        date_str = "<unknown>"
