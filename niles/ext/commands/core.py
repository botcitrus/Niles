...
if TYPE_CHECKING:
    ...
    from niles.message import Message
        ...
        try:
            ...
            if cog is not None:
                ...
                if local_check is not None:
                    ret = await niles.utils.maybe_coroutine(local_check, ctx)
                    if not ret:
                        return False
            ...
            return await niles.utils.async_all(predicate(ctx) for predicate in predicates)  # type: ignore
        finally:
...
def app_check(predicate: AppCheck) -> Callable[[T], T]:
    ...
    Parameters
    ----------
    predicate: Callable[[:class:`niles.ApplicationCommandInteraction`], :class:`bool`]
        The predicate to check if the command should be invoked.
    """
...
def app_check_any(*checks: AppCheck) -> Callable[[T], T]:
    ...
    Parameters
    ----------
    *checks: Callable[[:class:`niles.ApplicationCommandInteraction`], :class:`bool`]
        An argument list of checks that have been decorated with
...
def has_role(item: Union[int, str]) -> Callable[[T], T]:
    ...
    def predicate(ctx: AnyContext) -> bool:
        ...
        if isinstance(item, int):
            role = niles.utils.get(ctx.author.roles, id=item)  # type: ignore
        else:
            role = niles.utils.get(ctx.author.roles, name=item)  # type: ignore
        if role is None:
...
def has_any_role(*items: Union[int, str]) -> Callable[[T], T]:
    ...
    def predicate(ctx: AnyContext) -> bool:
        ...
        # ctx.guild is None doesn't narrow ctx.author to Member
        getter = functools.partial(niles.utils.get, ctx.author.roles)  # type: ignore
        if any(
...
def bot_has_role(item: int) -> Callable[[T], T]:
    ...
    def predicate(ctx: AnyContext) -> bool:
        ...
        me = cast(niles.Member, ctx.me)
        if isinstance(item, int):
            role = niles.utils.get(me.roles, id=item)
        else:
            role = niles.utils.get(me.roles, name=item)
        if role is None:
...
def bot_has_any_role(*items: int) -> Callable[[T], T]:
    ...
    def predicate(ctx: AnyContext) -> bool:
        ...
        me = cast(niles.Member, ctx.me)
        getter = functools.partial(niles.utils.get, me.roles)
        if any(
            getter(id=item) is not None if isinstance(item, int) else getter(name=item) is not None
            for item in items
        ):
...
def has_permissions(**perms: bool) -> Callable[[T], T]:
    ...
    """
    invalid = set(perms) - set(niles.Permissions.VALID_FLAGS)
    if invalid:
        raise TypeError(f"Invalid permission(s): {', '.join(invalid)}")

    def predicate(ctx: AnyContext) -> bool:
        if isinstance(ctx, niles.Interaction):
...
def dynamic_cooldown(
    ...
    accepts a single parameter of type :class:`.niles.Message` and must
    ...
    Parameters
    ----------
    cooldown: Callable[[:class:`.niles.Message`], Optional[:class:`.Cooldown`]]
