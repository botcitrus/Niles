...
import niles
from niles.abc import GuildChannel
...
class TestGuildChannelEdit:
    ...
    async def test_all(self, channel) -> None:
        mock_role = mock.Mock(niles.Role)
        mock_member = mock.Mock(niles.Member)
        ...
        res = await GuildChannel._edit(
            ...
            category=niles.Object(321),
            slowmode_delay=8,
            default_thread_slowmode_delay=42,
            default_auto_archive_duration=niles.ThreadArchiveDuration.hour,
            type=niles.ChannelType.news,
            overwrites={
                mock_role: niles.PermissionOverwrite(kick_members=False, send_messages=True),
                mock_member: niles.PermissionOverwrite(ban_members=False),
            },
            bitrate=42000,
            user_limit=3,
            rtc_region="there",
            video_quality_mode=niles.VideoQualityMode.full,
            flags=niles.ChannelFlags(pinned=False, require_tag=True),
            available_tags=[niles.ForumTag(name="tag", emoji="woo")],
            default_reaction=niles.PartialEmoji(name="woo", id=9876),
            default_sort_order=niles.ThreadSortOrder.creation_date,
            default_layout=niles.ThreadLayout.gallery_view,
            reason="stuff",
        )
    ...
    async def test_move_only(self, channel) -> None:
        ...
        res = await GuildChannel._edit(
            channel, position=42, category=niles.Object(999), sync_permissions=True, reason="h"
        )
        assert res is None
...
class TestUserProtocol:
    def _test_typing_assignable(self) -> None:
        def handle_abc_user(user: niles.abc.User) -> None:
            ...
        ...
        handle_abc_user(cast(niles.User, ...))
        handle_abc_user(cast(niles.ClientUser, ...))
        handle_abc_user(cast(niles.Member, ...))
