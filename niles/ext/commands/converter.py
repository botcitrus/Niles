...
if TYPE_CHECKING:
    from niles.abc import MessageableChannel
...
_utils_get = niles.utils.get
T = TypeVar("T")
T_co = TypeVar("T_co", covariant=True)
CT = TypeVar("CT", bound=niles.abc.GuildChannel)
TT = TypeVar("TT", bound=niles.Thread)


def _get_from_guilds(
    client: niles.Client, func: Callable[[niles.Guild], Optional[T]]
) -> Optional[T]:
...
class ObjectConverter(IDConverter[niles.Object]):
    """Converts to a :class:`~niles.Object`.
    ...
    async def convert(self, ctx: AnyContext, argument: str) -> niles.Object:
        ...
        return niles.Object(id=result)


class MemberConverter(IDConverter[niles.Member]):
    """Converts to a :class:`~niles.Member`.
    ...
    async def query_member_named(
        self, guild: niles.Guild, argument: str
    ) -> Optional[niles.Member]:
    ...
    async def convert(self, ctx: AnyContext, argument: str) -> niles.Member:
        bot: niles.Client = ctx.bot
        match = self._get_id_match(argument) or re.match(r"<@!?([0-9]{17,19})>$", argument)
        guild = ctx.guild
        result: Optional[niles.Member] = None
        ...
        if match is None:
            ...
            if guild:
                mentions: Iterable[niles.Member]
                if isinstance(ctx, Context):
                    mentions = (
                        user for user in ctx.message.mentions if isinstance(user, niles.Member)
                    )
                else:
                    mentions = []
...
class UserConverter(IDConverter[niles.User]):
    """Converts to a :class:`~niles.User`.
    ...
    .. versionchanged:: 2.9
        Now takes :attr:`~niles.User.global_name` into account.
        No longer automatically removes ``"@"`` prefix from arguments.
    """

    async def convert(self, ctx: AnyContext, argument: str) -> niles.User:
        match = self._get_id_match(argument) or re.match(r"<@!?([0-9]{17,19})>$", argument)
        state = ctx._state
        bot: niles.Client = ctx.bot
        result: Optional[Union[niles.User, niles.Member]] = None

        if match is not None:
            user_id = int(match.group(1))

            mentions: Iterable[Union[niles.User, niles.Member]]
            if isinstance(ctx, Context):
            ...
            if isinstance(result, niles.Member):
                return result._user
            return result
...
class PartialMessageConverter(Converter[niles.PartialMessage]):
    """Converts to a :class:`~niles.PartialMessage`.
    ...
    def _get_id_matches(ctx: AnyContext, argument: str) -> Tuple[Optional[int], int, int]:
        ...
        channel_id = niles.utils._get_as_snowflake(data, "channel_id") or ctx.channel.id
    ...
    async def convert(self, ctx: AnyContext, argument: str) -> niles.PartialMessage:
        guild_id, message_id, channel_id = self._get_id_matches(ctx, argument)
        channel = self._resolve_channel(ctx, guild_id, channel_id)
        if not channel:
            raise ChannelNotFound(str(channel_id))
        return niles.PartialMessage(channel=channel, id=message_id)


class MessageConverter(IDConverter[niles.Message]):
    """Converts to a :class:`~niles.Message`.
    ...
    async def convert(self, ctx: AnyContext, argument: str) -> niles.Message:
        ...
        except niles.NotFound:
            raise MessageNotFound(argument) from None
        except niles.Forbidden:
            raise ChannelNotReadable(channel) from None  # type: ignore


class GuildChannelConverter(IDConverter[niles.abc.GuildChannel]):
    ...
    async def convert(self, ctx: AnyContext, argument: str) -> niles.abc.GuildChannel:
        return self._resolve_channel(ctx, argument, "channels", niles.abc.GuildChannel)
    ...
    def _resolve_channel(ctx: AnyContext, argument: str, attribute: str, type: Type[CT]) -> CT:
        ...
        result: Optional[niles.abc.GuildChannel] = None
        guild = ctx.guild
    ...
    def _resolve_thread(ctx: AnyContext, argument: str, attribute: str, type: Type[TT]) -> TT:
        match = IDConverter._get_id_match(argument) or re.match(r"<#([0-9]{17,19})>$", argument)
        result: Optional[niles.Thread] = None
        guild = ctx.guild
...
class TextChannelConverter(IDConverter[niles.TextChannel]):
    """Converts to a :class:`~niles.TextChannel`.
    ...
    async def convert(self, ctx: AnyContext, argument: str) -> niles.TextChannel:
        return GuildChannelConverter._resolve_channel(
            ctx, argument, "text_channels", niles.TextChannel
        )


class VoiceChannelConverter(IDConverter[niles.VoiceChannel]):
    """Converts to a :class:`~niles.VoiceChannel`.
    ...
    async def convert(self, ctx: AnyContext, argument: str) -> niles.VoiceChannel:
        return GuildChannelConverter._resolve_channel(
            ctx, argument, "voice_channels", niles.VoiceChannel
        )


class StageChannelConverter(IDConverter[niles.StageChannel]):
    """Converts to a :class:`~niles.StageChannel`.
    ...
    async def convert(self, ctx: AnyContext, argument: str) -> niles.StageChannel:
        return GuildChannelConverter._resolve_channel(
            ctx, argument, "stage_channels", niles.StageChannel
        )


class CategoryChannelConverter(IDConverter[niles.CategoryChannel]):
    """Converts to a :class:`~niles.CategoryChannel`.
    ...
    async def convert(self, ctx: AnyContext, argument: str) -> niles.CategoryChannel:
        return GuildChannelConverter._resolve_channel(
            ctx, argument, "categories", niles.CategoryChannel
        )


class ForumChannelConverter(IDConverter[niles.ForumChannel]):
    """Converts to a :class:`~niles.ForumChannel`.
    ...
    async def convert(self, ctx: AnyContext, argument: str) -> niles.ForumChannel:
        return GuildChannelConverter._resolve_channel(
            ctx, argument, "forum_channels", niles.ForumChannel
        )


class ThreadConverter(IDConverter[niles.Thread]):
    """Coverts to a :class:`~niles.Thread`.
    ...
    async def convert(self, ctx: AnyContext, argument: str) -> niles.Thread:
        return GuildChannelConverter._resolve_thread(ctx, argument, "threads", niles.Thread)


class ColourConverter(Converter[niles.Colour]):
    """Converts to a :class:`~niles.Colour`.
    ...
    - Any of the ``classmethod`` in :class:`~niles.Colour`
    ...
    def parse_hex_number(self, argument: str) -> niles.Color:
        ...
        try:
            ...
            return niles.Color(value=value)
    ...
    def parse_rgb(self, argument: str, *, regex: re.Pattern[str] = RGB_REGEX) -> niles.Color:
        ...
        return niles.Color.from_rgb(red, green, blue)

    async def convert(self, ctx: AnyContext, argument: str) -> niles.Color:
        if argument[0] == "#":
        ...
        arg = arg.replace(" ", "_")
        method = getattr(niles.Colour, arg, None)
...
class RoleConverter(IDConverter[niles.Role]):
    """Converts to a :class:`~niles.Role`.
    ...
    async def convert(self, ctx: AnyContext, argument: str) -> niles.Role:
        guild = ctx.guild
        if not guild:
...
class GameConverter(Converter[niles.Game]):
    """Converts to :class:`~niles.Game`."""

    async def convert(self, ctx: AnyContext, argument: str) -> niles.Game:
        return niles.Game(name=argument)


class InviteConverter(Converter[niles.Invite]):
    """Converts to a :class:`~niles.Invite`.
    ...
    async def convert(self, ctx: AnyContext, argument: str) -> niles.Invite:
        try:
...
class GuildConverter(IDConverter[niles.Guild]):
    """Converts to a :class:`~niles.Guild`.
    ...
    async def convert(self, ctx: AnyContext, argument: str) -> niles.Guild:
        match = self._get_id_match(argument)
        bot: niles.Client = ctx.bot
        result: Optional[niles.Guild] = None
...
class EmojiConverter(IDConverter[niles.Emoji]):
    """Converts to a :class:`~niles.Emoji`.
    ...
    async def convert(self, ctx: AnyContext, argument: str) -> niles.Emoji:
        match = self._get_id_match(argument) or re.match(
            r"<a?:[a-zA-Z0-9\_]{1,32}:([0-9]{17,19})>$", argument
        )
        result: Optional[niles.Emoji] = None
        bot = ctx.bot
        guild = ctx.guild
...
class PartialEmojiConverter(Converter[niles.PartialEmoji]):
    """Converts to a :class:`~niles.PartialEmoji`.
    ...
    async def convert(self, ctx: AnyContext, argument: str) -> niles.PartialEmoji:
        ...
        if match:
            ...
            return niles.PartialEmoji.with_state(
...
class GuildStickerConverter(IDConverter[niles.GuildSticker]):
    """Converts to a :class:`~niles.GuildSticker`.
    ...
    async def convert(self, ctx: AnyContext, argument: str) -> niles.GuildSticker:
        match = self._get_id_match(argument)
        result = None
        bot: niles.Client = ctx.bot
        guild = ctx.guild
...
class PermissionsConverter(Converter[niles.Permissions]):
    """Converts to a :class:`~niles.Permissions`.
    ...
    async def convert(self, ctx: AnyContext, argument: str) -> niles.Permissions:
        ...
        for name in argument.split():
            ...
            if callable(attr):
                ...
                perms.append(niles.Permissions(**{name: True}))
        else:
            return functools.reduce(lambda a, b: niles.Permissions(a.value | b.value), perms)
        ...
        if callable(attr):
            ...
            return niles.Permissions(**{name: True})


class GuildScheduledEventConverter(IDConverter[niles.GuildScheduledEvent]):
    """Converts to a :class:`~niles.GuildScheduledEvent`.
    ...
    async def convert(self, ctx: AnyContext, argument: str) -> niles.GuildScheduledEvent:
        event_regex = re.compile(
            r"https?://(?:(?:ptb|canary|www)\.)?discord(?:app)?\.com/events/"
            r"([0-9]{17,19})/([0-9]{17,19})/?$"
        )
        bot: niles.Client = ctx.bot
        result: Optional[niles.GuildScheduledEvent] = None
        guild = ctx.guild
...
class clean_content(Converter[str]):
    ...
    This behaves similarly to :attr:`~niles.Message.clean_content`.
    ...
    async def convert(self, ctx: AnyContext, argument: str) -> str:
        ...
        if self.escape_markdown:
            result = niles.utils.escape_markdown(result)
        elif self.remove_markdown:
            result = niles.utils.remove_markdown(result)

        # Completely ensure no mentions escape:
        return niles.utils.escape_mentions(result)
...
CONVERTER_MAPPING: Dict[Type[Any], Type[Converter]] = {
    niles.Object: ObjectConverter,
    niles.Member: MemberConverter,
    niles.User: UserConverter,
    niles.Message: MessageConverter,
    niles.PartialMessage: PartialMessageConverter,
    niles.TextChannel: TextChannelConverter,
    niles.Invite: InviteConverter,
    niles.Guild: GuildConverter,
    niles.Role: RoleConverter,
    niles.Game: GameConverter,
    niles.Colour: ColourConverter,
    niles.VoiceChannel: VoiceChannelConverter,
    niles.StageChannel: StageChannelConverter,
    niles.Emoji: EmojiConverter,
    niles.PartialEmoji: PartialEmojiConverter,
    niles.CategoryChannel: CategoryChannelConverter,
    niles.ForumChannel: ForumChannelConverter,
    niles.Thread: ThreadConverter,
    niles.abc.GuildChannel: GuildChannelConverter,
    niles.GuildSticker: GuildStickerConverter,
    niles.Permissions: PermissionsConverter,
    niles.GuildScheduledEvent: GuildScheduledEventConverter,
}
...
async def _actual_conversion(
    ...
    if isinstance(converter, type):
        ...
        if module.startswith("niles.") and not module.endswith("converter"):
