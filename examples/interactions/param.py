...
# niles will parse the signatures of commands,
...
@bot.slash_command()
async def simple(
    inter: niles.CommandInteraction,
    required: str,
    optional: int = 0,
):
    ...
...
async def other_types(
    ...
    user: niles.User,
    emoji: niles.Emoji,
    member_or_role: Union[niles.Member, niles.Role],
):
    ...
...
async def defaults(
    ...
    user: niles.User = commands.Param(lambda inter: inter.author),
):
    ...
...
@bot.slash_command()
async def large(inter: niles.CommandInteraction, largenumber: commands.LargeInt):
    ...
