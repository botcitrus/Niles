...
- ``Channel.is_private``
    ...
    - e.g. ``isinstance(channel, niles.abc.GuildChannel)`` will check if it isn't a private channel.
...
- :class:`Attachment` to represent a niles attachment.
...
file attachments, you must now use a :class:`File` pseudo-namedtuple to upload a single file. ::
    ...
    # after
    await channel.send('Hello', file=niles.File('cool.png', 'testing.png'))

This change was to facilitate multiple file uploads: ::

    my_files = [
        niles.File('cool.png', 'testing.png'),
        niles.File(some_fp, 'cool_filename.png'),
    ]
...
You can also get single elements a la :func:`niles.utils.find` or :func:`niles.utils.get` via
...
After: ::
    ...
    vc.play(niles.FFmpegPCMAudio('testing.mp3'), after=lambda e: print('done', e))
    vc.is_playing()
    vc.pause()
    vc.resume()
    vc.stop()
...
For example, you can add a :class:`PCMVolumeTransformer` to allow changing the volume: ::
    ...
    vc.source = niles.PCMVolumeTransformer(vc.source)
...
Usage is as simple as doing: ::
    ...
    client = niles.AutoShardedClient()
...
If you want more control over the sharding you can specify ``shard_count`` and ``shard_ids``. ::
    ...
    client = niles.AutoShardedClient(shard_count=10)

    # launch specific shard IDs in this process
    client = niles.AutoShardedClient(shard_count=10, shard_ids=(1, 2, 5, 6))
...
as the built-in "niles" converters.
