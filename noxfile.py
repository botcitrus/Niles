...
def check_manifest(session: nox.Session) -> None:
    """Run check-manifest."""
    # --no-self is provided here because check-manifest builds niles. There's no reason to build twice, so we don't.
...
def slotscheck(session: nox.Session) -> None:
    ...
    session.run("python", "-m", "slotscheck", "--verbose", "-m", "niles")
...
def codemod(session: nox.Session) -> None:
    ...
    if (session.posargs and session.posargs[0] == "run-all") or not session.interactive:
        # run all of the transformers on niles
        session.log("Running all transformers.")

        session.run(*base_command_codemod, "combined.CombinedCodemod", "niles")
    elif session.posargs:
        if len(session.posargs) < 2:
            session.posargs.append("niles")
