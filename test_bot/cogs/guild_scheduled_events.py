...
import niles
from niles.enums import GuildScheduledEventEntityType, GuildScheduledEventPrivacyLevel
...
class GuildScheduledEvents(commands.Cog):
    ...
    async def fetch_event(
        self, inter: niles.GuildCommandInteraction[commands.Bot], id: commands.LargeInt
    ) -> None:
    ...
    async def edit_event(
        self,
        inter: niles.GuildCommandInteraction[commands.Bot],
        id: commands.LargeInt,
        new_image: bool,
    ) -> None:
        await inter.response.defer()
        gse = await inter.guild.fetch_scheduled_event(id)
        image = niles.File("./assets/banner.png")
        if new_image:
