...
- Ensure ``niles`` package is only run if executed as a script (:issue-dpy:`6483`)
...
- ``_`` and ``-`` characters are now stripped when making a new cog using the ``niles`` package (:issue-dpy:`6313`)
...
- |commands| :class:`MemberConverter <ext.commands.MemberConverter>` now properly lazily fetches members if not available from cache.
    - This is the same as having ``niles.Member`` as the type-hint.
...
- Make the output of ``python -m niles --version`` a bit more useful.
...
- New utility function, :func:`niles.utils.get` to simplify retrieval of items based on attributes.
...
- Add :class:`niles.Game` to represent any game with custom text to send to :meth:`Client.change_status`.
...
- :meth:`Client.logs_from` now handles cases where limit > 100 to sidestep the niles API limitation.
...
- Mentions are now triggered normally. This was changed due to the way niles handles it internally.
