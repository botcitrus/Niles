...
You may want to define a couple of options for your slash command. In niles, the definition of options is based on annotations.
...
- :class:`float`
- :class:`bool`
- :class:`niles.abc.GuildChannel`\*
- :class:`niles.User` or :class:`niles.Member`\*\*
- :class:`niles.Role`\*\*
- :class:`niles.Attachment`
- :class:`niles.abc.Snowflake`\*\*\*
...
.. note::
  ...
  Note that a :class:`~niles.User` annotation can also result in a :class:`~niles.Member` being received.
...
.. code-block:: python3
    ...
    @bot.slash_command()
    async def command(
        inter: niles.ApplicationCommandInteraction,
...
Python has no truly *clean* way to provide metadata for parameters, so niles uses the same approach as fastapi using
parameter defaults. At the current time there's only :class:`~niles.ext.commands.Param`.
...
.. code-block:: python3
    ...
    @bot.slash_command()
    async def math(
        interaction: niles.ApplicationCommandInteraction,
...
.. note ::
    ...
    Converter classes are completely unusable in niles due to their inconsistent typing.
...
.. code-block:: python3
    ...
    @bot.slash_command()
    async def blep(inter: niles.ApplicationCommandInteraction, animal: Animal):
...
.. code-block:: python3
    ...
    async def autocomp_langs(inter: niles.ApplicationCommandInteraction, user_input: str):
...
.. code-block:: python3
    ...
    @bot.slash_command()
    async def languages(inter: niles.ApplicationCommandInteraction, language: str):
        pass


    @languages.autocomplete("language")
    async def language_autocomp(inter: niles.ApplicationCommandInteraction, string: str):
        string = string.lower()
...
.. code-block:: python3
    ...
    @show.sub_command()
    async def user(inter, user: niles.User):
        """
...
We have them, look at `this example <https://github.com/botcitrus/niles/blob/master/examples/interactions/injections.py>`__ for more information ✨
...
.. code-block:: python3
    ...
    @bot.slash_command()
    async def add_5(inter: niles.ApplicationCommandInteraction, num: int):
        """
...
To load a directory or file containing localizations, use :func:`bot.i18n.load(path) <niles.LocalizationStore.load>`:
...
.. code-block:: python3
    ...
    async def language_autocomp(inter: niles.ApplicationCommandInteraction, user_input: str):
...
.. code-block:: python3
    ...
    async def command(inter: niles.ApplicationCommandInteraction):
        ...
...
.. code-block:: python3
    ...
    @bot.slash_command(default_member_permissions=niles.Permissions(manage_guild=True, moderate_members=True))
    async def command(inter: niles.ApplicationCommandInteraction):
        ...
...
.. code-block:: python3
    ...
    async def config(inter: niles.ApplicationCommandInteraction):
        ...
