...
import niles
from niles import InteractionResponseType as ResponseType  # shortcut
from niles.state import ConnectionState
from niles.utils import MISSING

if TYPE_CHECKING:
    from niles.types.interactions import ResolvedPartialChannel as ResolvedPartialChannelPayload
    from niles.types.member import Member as MemberPayload
    from niles.types.user import User as UserPayload
...
class TestInteractionResponse:
    @pytest.fixture
    def response(self):
        inter = mock.Mock(niles.Interaction)
        return niles.InteractionResponse(inter)

    @pytest.fixture
    def adapter(self):
        adapter = mock.AsyncMock()
        niles.interactions.base.async_context.set(adapter)
        return adapter
    ...
    @pytest.mark.parametrize(
        ...
        [
            # application_command
            (
                niles.InteractionType.application_command,
                MISSING,
            ...
            # component
            (
                niles.InteractionType.component,
                MISSING,
            ...
            # modal_submit
            (
                niles.InteractionType.modal_submit,
                MISSING,
                {"type": ResponseType.deferred_channel_message.value, "data": {"flags": 0}},
            ),
            (
    ...
    async def test_defer(
        self, response: niles.InteractionResponse, adapter, parent_type, with_message, expected
    ) -> None:
    ...
    async def test_defer_ephemeral(
        self, response: niles.InteractionResponse, adapter, with_message, expected_data
    ) -> None:
        response._parent.type = niles.InteractionType.component
    ...
    async def test_defer_invalid_parent(
        self, response: niles.InteractionResponse, adapter
    ) -> None:
        # autocomplete can't be deferred
        response._parent.type = niles.InteractionType.application_command_autocomplete
...
class TestInteractionDataResolved:
    ...
    def test_init_member(self, state) -> None:
        ...
        resolved = niles.InteractionDataResolved(
    ...
    @pytest.mark.parametrize("channel_type", [t.value for t in niles.ChannelType])
    def test_channel(self, state, channel_type) -> None:
        ...
        assert isinstance(channel, niles.PartialMessageable) == (channel_type in (1, 3, 14))
