...
from niles import Color, Embed, File, embeds
from niles.utils import MISSING, utcnow
...
def test_embed_proxy_eq() -> None:
    ...
    embed_1.set_image("https://niles.dev/assets/niles-logo.png")
    embed_2.set_image(None)
    assert not embed_1.image == embed_2.image

    embed_2.set_image("https://niles.dev/assets/niles-logo.png")
...
def test_image_url() -> None:
    embed = Embed()
    embed.set_image("https://niles.dev")
    assert embed._files == {}
    assert embed.to_dict() == {"image": {"url": "https://niles.dev"}, **_BASE}
...
def test_file_params(file: File) -> None:
    ...
    with pytest.raises(TypeError):
        embed.set_image("https://niles.dev/assets/niles-logo.png", file=file)  # type: ignore
