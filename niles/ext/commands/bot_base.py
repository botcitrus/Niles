...
class BotBase(CommonBotBase, GroupMixin):
    def __init__(
        ...
        **options: Any,
        ...
        if not isinstance(self, niles.Client):
            raise RuntimeError("BotBase mixin must be used with niles.Client")  # noqa: TRY004

        alternative = (
            "AutoShardedInteractionBot"
            if isinstance(self, niles.AutoShardedClient)
            else "InteractionBot"
        )
        if command_prefix is None:
            ...
            warnings.warn(
                ...
                f"If you don't need any prefix functionality, consider using {alternative}. "
                "Alternatively, set prefix to niles.ext.commands.when_mentioned to silence this warning.",
    ...
    async def can_run(self, ctx: Context, *, call_once: bool = False) -> bool:
        ...
        return await niles.utils.async_all(f(ctx) for f in data)  # type: ignore
    ...
    async def get_prefix(self, message: Message) -> Optional[Union[List[str], str]]:
        ...
        if callable(ret):
            ret = await niles.utils.maybe_coroutine(ret, self, message)
        ...
        if prefix is None:
            ...
            try:
                ...
                if message.content.startswith(tuple(prefix)):
                    invoked_prefix = niles.utils.find(view.skip_string, prefix)
                else:
                    return ctx
