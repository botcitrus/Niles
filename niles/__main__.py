...
def show_version() -> None:
    ...
    disnake_ver = niles.version_info
    entries.append(
        f"- niles v{disnake_ver.major}.{disnake_ver.minor}.{disnake_ver.micro}-{disnake_ver.releaselevel}"
    )
    try:
        version = importlib.metadata.version("niles")
    except importlib.metadata.PackageNotFoundError:
        pass
    else:
        entries.append(f"    - niles importlib.metadata: v{version}")
...
_cog_template = '''from niles.ext import commands
import niles
...
def parse_args():
    parser = argparse.ArgumentParser(prog="niles", description="Tools for helping with niles")
