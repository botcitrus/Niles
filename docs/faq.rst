...
This is a list of Frequently Asked Questions regarding using ``niles`` and its extension modules. Feel free to suggest a
...
Example code can be found in the `examples folder <https://github.com/botcitrus/niles/tree/master/examples>`_
in the repository.
...
Putting both of these pieces of info together, you get the following: ::
    ...
    client = niles.Client(activity=niles.Game(name='my game'))

    # alternatively, a plain custom status:
    client = niles.Client(activity=niles.CustomActivity(name='As seen on TV!'))

    # or, for watching:
    activity = niles.Activity(name='my activity', type=niles.ActivityType.watching)
    client = niles.Client(activity=activity)
...
If you want to upload an image it's as simple as: ::
    ...
    await channel.send(file=niles.File('my_file.png'))

If you have a file-like object you can do as follows: ::

    with open('my_file.png', 'rb') as fp:
        await channel.send(file=niles.File(fp, 'new_filename.png'))

To upload multiple files, you can use the ``files`` keyword argument instead of ``file``\: ::

    my_files = [
        niles.File('result.zip'),
        niles.File('teaser_graph.png'),
    ]
...
.. code-block:: python3
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(my_url) as resp:
            ...
            await channel.send(file=niles.File(data, 'cool_image.png'))
...
Quick example: ::
    ...
    # no ID, do a lookup
    emoji = niles.utils.get(guild.emojis, name='LUL')
    if emoji:
        await message.add_reaction(emoji)
...
this together we can do the following: ::
    ...
    voice.play(niles.FFmpegPCMAudio(url), after=my_after)

How do I run something in the background?
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

`Check the background_task.py example. <https://github.com/botcitrus/niles/blob/master/examples/background_task.py>`_
...
Quick example: ::
    ...
    guild = niles.utils.get(client.guilds, name='My Server')

    # make sure to check if it's found
    if guild is not None:
        # find a channel by name
        channel = niles.utils.get(guild.text_channels, name='cool-channel')
...
Quick example: ::
    ...
    file = niles.File("path/to/my/image.png", filename="image.png")
    embed = niles.Embed()
...
Questions regarding ``niles.ext.commands`` belong here.
