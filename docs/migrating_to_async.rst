...
.. code-block:: python3
    ...
    server.region == niles.ServerRegion.us_west
    member.status = niles.Status.online
    channel.type == niles.ChannelType.text
...
In earlier versions of niles, ``client.run()`` was a blocking call to the main thread
