...
- |commands| :class:`~niles.ext.commands.Range` and :class:`~niles.ext.commands.String` now require a type argument (i.e. ``Range[int, 1, 5]`` instead of ``Range[1, 5]``, similarly with ``String[str, 2, 4]``). The old form is deprecated. (:issue:`991`)
...
- |commands| Add :meth:`Bot.get_listeners() <niles.ext.commands.Bot.get_listeners>`. (:issue:`976`)
...
- |commands| Fix type-checker support for :class:`~niles.ext.commands.Range` and :class:`~niles.ext.commands.String` by requiring type argument (i.e. ``Range[int, 1, 5]`` instead of ``Range[1, 5]``). (:issue:`991`)
...
- Split the monolithic API Reference pages into multiple sub-references. (:issue:`392`)
    - Main ``niles`` API reference can now be found at :doc:`API Reference <api/index>`.
    - ``niles.ext.commands`` API reference is now under :doc:`Commands API Reference <ext/commands/api/index>`.
    - Legacy ``api.html`` and ``ext/commands/api.html`` pages are deprecated.
    - - Links with pre-existing references (eg ``/api.html#niles.AppInfo``) will be redirected to their appropriate page.
...
- Change dependency and environment management to use `pdm <https://pdm.fming.dev/>`__. (:issue:`836`, :issue:`953`)
    Please check `CONTRIBUTING.md <https://github.com/botcitrus/niles/tree/master/CONTRIBUTING.md>`__ for more details.
...
- Remove :func:`niles.utils.parse_token` (never documented), which has been broken for newer tokens for some time, and was based on unofficial information about the token structure. (:issue:`990`)
...
- Adds :class:`Event` enumeration to use in :meth:`Client.wait_for`, :meth:`niles.ext.commands.Bot.wait_for` and in :func:`niles.ext.commands.Bot.listen` decorator. (:issue:`895`)
...
- |commands| Deprecate the ``sync_commands``, ``sync_commands_debug``, and  ``sync_commands_on_cog_unload`` parameters of :class:`~niles.ext.commands.Bot` and :class:`~niles.ext.commands.InteractionBot`. These have been replaced with the ``command_sync_flags`` parameter which takes a :class:`~niles.ext.commands.CommandSyncFlags` instance. (:issue:`806`)
...
- |commands| Add :class:`~niles.ext.commands.CommandSyncFlags` to provide sync configuration to :class:`~niles.ext.commands.Bot` and :class:`~niles.ext.commands.InteractionBot` (and their autosharded variants) as ``command_sync_flags``. (:issue:`265`, :issue:`433`, :issue:`468`, :issue:`806`)
...
- Add a note warning mentioning that using a :class:`niles.File` object as file kwarg makes a :class:`niles.Embed` not reusable. (:issue:`786`)
...
- Call new :func:`niles.on_gateway_error` instead of letting exceptions propagate that occurred while deserializing a received gateway event. (:issue:`401`)
...
- |commands| Using ``command_prefix=None`` with :class:`~niles.ext.commands.Bot` is now deprecated in favour of :class:`~niles.ext.commands.InteractionBot`. (:issue:`689`)

New Features
~~~~~~~~~~~~

- Add custom type support for :func:`niles.ui.button` and :func:`niles.ui.select` decorators using ``cls`` parameter. (:issue:`281`)
- Add :func:`niles.on_gateway_error`, :func:`Client.on_gateway_error` and ``enable_gateway_error_handler`` client parameter. (:issue:`401`)
...
- Add permission typings to all methods that take permissions directly, for example :func:`niles.abc.GuildChannel.set_permissions` and :func:`niles.ext.commands.bot_has_permissions` to name a few. (:issue:`708`)
...
- Return the :class:`niles.ui.View` instance from :func:`View.add_item <niles.ui.View.add_item>`, :func:`View.remove_item <niles.ui.View.remove_item>` and :func:`View.clear_items <niles.ui.View.clear_items>` to allow for fluent-style chaining. (:issue:`733`)
...
- Return the :class:`niles.ui.ActionRow` instance on multiple methods to allow for fluent-style chaining. (:issue:`740`)
    - This applies to :func:`ActionRow.append_item <niles.ui.ActionRow.append_item>`, :func:`ActionRow.insert_item <niles.ui.ActionRow.insert_item>`, :func:`ActionRow.add_button <niles.ui.ActionRow.add_button>`, :func:`ActionRow.add_select <niles.ui.ActionRow.add_select>`, :func:`ActionRow.add_text_input <niles.ui.ActionRow.add_text_input>`, :func:`ActionRow.clear_items <niles.ui.ActionRow.clear_items>`, and :func:`ActionRow.remove_item <niles.ui.ActionRow.remove_item>`.
- Add support for equality checks between two :class:`niles.Embed`\s. (:issue:`742`)
...
- |commands| Introduce :class:`commands.String <niles.ext.commands.String>` for defining string option length limitations. (:issue:`593`)
...
- Dispatch :func:`niles.on_reaction_remove` for :class:`.Thread` instances. (:issue:`536`)
...
- Clarify when the user is a :class:`Member` or a :class:`User` in :func:`niles.on_member_ban` events. (:issue:`623`)
...
- |commands| Improve documentation around using ``None`` for :attr:`Bot.command_prefix <niles.ext.commands.Bot.command_prefix>`. (:issue:`689`)
...
- Rename internal module ``niles.ext.commands.flags`` to ``niles.ext.commands.flag_converter``. (:issue:`667`)
...
- Update annotation and description of ``options`` parameter of :func:`ui.ActionRow.add_select <niles.ui.ActionRow.add_select>` to match :class:`ui.Select <niles.ui.Select>`. (:issue:`744`)
...
See `this page <https://guide.niles.dev/popular-topics/intents#why-do-most-messages-have-no-content>`_ of the guide for more information.
If you do not have access to the intent yet, you can temporarily continue using API v9 by calling ``niles.http._workaround_set_api_version(9)`` before connecting,
...
- Support ``List[str]`` and ``Dict[str, str]`` in ``option`` parameter of :class:`niles.ui.Select` (:issue:`326`)
...
- Fix shadowed ``niles.message`` module (:issue:`380`)
...
- Make ``niles.types.interactions`` importable at runtime (:issue:`493`)
...
- |commands| Add :class:`commands.Range <niles.ext.commands.Range>`, see :ref:`param_ranges` (:issue:`237`, :issue:`276`, :issue:`316`)
- |commands| Add :func:`Bot.load_extensions <niles.ext.commands.Bot.load_extensions>` and :func:`utils.search_directory` (:issue:`245`)
- |commands| Add :class:`commands.LargeInt <niles.ext.commands.LargeInt>` (:issue:`264`)
...
- Support :class:`niles.Attachment` options in slash commands (:issue:`226`)
...
- |commands| Fix typing of ``ctx`` parameter in :class:`~niles.ext.commands.Converter` (:issue:`292`)
...
- Fix :func:`tasks.loop <niles.ext.tasks.loop>` usage with fixed times (:issue:`337`)
...
- ``niles.types.ThreadArchiveDuration`` is now ``ThreadArchiveDurationLiteral``, to avoid confusion with the new :class:`ThreadArchiveDuration` enum
...
- |commands| Add parameter injections (`example <https://github.com/botcitrus/niles/blob/master/examples/interactions/injections.py>`__) (:issue:`130`)
...
- Move documentation to https://docs.niles.dev/
...
- Fix :class:`Select.append_option <niles.ui.Select.append_option>` not raising an error if 25 options have already been added
